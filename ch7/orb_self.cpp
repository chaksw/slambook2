/*
 * @Author: Runsheng WU 
 * @Date: 2020-07-09 17:39:29 
 * @Last Modified by: Runsheng WU
 * @Last Modified time: 2020-07-10 23:42:13
 */
 ...
 * 
 * NOTE: if a keypoints goes outside the image boudary (8 pixels), 
 ...
 * @param mathces matches of two images
  */
...
{
    // load image
    Mat first_image = imread(first_file, 0);
    Mat second_image = imread(second_file, 0);
    assert(first_image.data != nullptr && second_image.data != nullptr);

    // detect FAST keypoint1 using threshold = 40
    chrono::steady_clock::time_point t1 = chrono::steady_clock::now();
    vector<KeyPoint> keypoints1;
    FAST(first_image, keypoints1, 40);      // 通过计算特征点附近的图像灰度质心，使得FAST角点获得尺度和旋转的描述
    vector<DescType> descriptor1;
    ComputeORB(first_image, keypoints1, descriptor1);   // 根据提取的FAST关键点，对每个点计算其描述子
    ...
    BfMatch(descriptor1, descriptor2, matches);     // 在得到两张图的描述子后，进行特征匹配
...
void ComputeORB(const Mat &img, vector<KeyPoint> &keypoints, vector<DescType> &descriptors)
{
    ...
    {   // 
        ...
        // comput the angle of this point
        DescType desc(8, 0);
        for (int i = 0; i < 8; i++)
        {
            uint32_t d = 0;
